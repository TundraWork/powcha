...
func (message Message) IsValidResponse() bool {
	algo, ok := algorithm.AlgorithmFromString(message.Algorithm)
	if !ok {
		return false
	}
	...
	if message.Challenge != internal.GenerateHash(algo, message.Salt, message.Number) {
		return false
	}
...
func (message Message) Solve(maximumComplexity int) (number int, ok bool) {
	if maximumComplexity <= 0 {
		maximumComplexity = config.Conf.Altcha.Complexity * 2
	}
	...
	for i := 1; i <= maximumComplexity; i++ {
		if message.Challenge == internal.GenerateHash(algo, message.Salt, i) {
			return i, true
		}
	}
