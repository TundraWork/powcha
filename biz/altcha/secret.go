...
// RotateSecrets immediately generates a new secret and replaces the previous secret with the current secret.
// This is concurrency safe and will block until complete.
func RotateSecrets() {
	secretsMutex.Lock()
...
func rotateSecrets() {
	...
	currentSecret = internal.RandomString(32)
...
// SetSecretsRotationInterval sets the interval at which secrets are automatically rotated.
// Setting the interval to 0 will disable automatic rotation.
...
// AddSecretsRotationCallback adds a callback function which is called when the secrets are rotated.
// It is run in a separate goroutine, so that the mutex is not held or locked when the callback is run.
